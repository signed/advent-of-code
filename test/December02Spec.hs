module December02Spec where
import Test.Hspec
import Data.Char(digitToInt)
import December02(Row, SpreadSheet, spreadsheetChecksum)

toSpreadsheet :: String -> SpreadSheet
toSpreadsheet x = map toRow rawSpreadsheet
                 where
                   toRow :: [String] -> Row
                   toRow a = map read a
                   rawSpreadsheet = (map words $ lines x) :: [[String]]

spec :: Spec
spec =
  describe "sumOfDigitsMatchingSuccessor for" $ do
    it " an emtpy list" $ do
      let input = "1 2\n\
                  \1 9"
      let sp = toSpreadsheet input
      spreadsheetChecksum sp `shouldBe` 9
    it "challenge" $ do
       ...
       let sp = toSpreadsheet input
       spreadsheetChecksum sp `shouldBe` 53460
